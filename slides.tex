...
\begin{frame}{Workflow}
  \begin{enumerate}
    \item Write (multiple) \texttt{requirements.in} files.
    \item Use \texttt{pip-compile} from \texttt{pip-tools}. See also \texttt{pip-compile-multi}.
    \item Resulting \texttt{requirements.txt} is \emph{checked into version control}.
    \item Do this for \emph{everything} this is needed for your builds.
    \item From time to time, go to 2.)
    \begin{itemize}
    ...
    \end{itemize}
  \item Make patch-releases ``quickly'' when a dependency update causes issues for users
    \begin{itemize}
...
\begin{frame}{Does it work?}
  \begin{itemize}
    \item We do \emph{not} have long term experience with this yet:
      \begin{itemize}
        \item So far implemented only in \url{https://github.com/scipp/scippnexus}
...
\begin{frame}{Case study: Should we freeze dependencies in PR builds?}
  ...
  \begin{itemize}
    ...
    \item The builds do not pass, since a completely unrelated test is failing (because there was a unrelated dependency update).
      Alice is confused.
    \item Bob, the project maintainer figures out that what the issue is, explains it in the PR, fixed the issue, merges into \texttt{main}, merges \texttt{main} into the PR, explains that all is good now, \dots
  ...
  \end{itemize}
  \begin{block}{Better solution: Freeze all dependencies for PR builds to \emph{decouple}}
    \begin{itemize}
